    ...
    if 'G1 ' in line:
        if 'X' in line:
            if 'E' in line:
                ...
                if 'Z' in line:
                    Z = float(extract_value_from_gcode(line, 'Z'))
                if 'E' in line:
                    E = float(extract_value_from_gcode(line, 'E'))
                
                    
                offset_1 = 1
                if line_idx +2 < len(gcode):
                    if 'G0' in gcode[line_idx + 2]:             #the problem with the offset while changing the layers is solved by
                        offset_1 = 0
                    
                X_steps = np.round(np.linspace(last_X, X, delta + offset_1), 3)
                Y_steps = np.round(np.linspace(last_Y, Y, delta + offset_1), 3)
                Z_steps = np.round(np.linspace(last_Z, Z, delta + offset_1), 3)
                E_steps = np.round(np.linspace(last_E, E, delta + offset_1), 5)             
                ...
                for line_idx, i in enumerate(range(0, int(delta))):                 #write the extruder T0,T1,T2 and if it always the same it passes over 
                    if ex_list2[i-1] == 0:
                        ...
                        if ex_list2[i-2] == 2 and i>0:
                            ...
                            outfile.write('T2;' + '\n')
                    new_line = line.replace('X' + str(X), 'X' + str(X_steps[line_idx]))         #replace the value in line with the new value that is the value divided into delta steps with the proper index
                    new_line = new_line.replace('Y' + str(Y),'Y' + str(Y_steps[line_idx]))
                    new_line = new_line.replace('Z' + str(Z),'Z' + str(Z_steps[line_idx]))
                    new_line = new_line.replace('E' + str(E),'E' + str(E_steps[line_idx]))
