...
"       Transiction phase is to add the part found on Internet (virtual tools T0-T18 and mix ratios)"
...
while sum(a) != 100:
    ...
    i = input(
        "How much of each extruder do you want to use? Please insert up to 3 values with a space in between" + '\n')
    a = list(float(s) for s in i.split())
print(a)
...
#     print(i)
n_file = 'Circular'
file = open('Circularone.gcode', 'r', encoding="utf8")
gcode = file.readlines()
b = open("Diamond HE.gcode", "w+")
if exp == 'Y':
    b.write('M163 S0 P1' + '\n' +
            'M163 S1 P0' + '\n' +
            'M163 S2 P0' + '\n' +
...
for line in gcode:
    if z1 == 'Y':
        if 'G1 ' in line or 'G02' in line or 'G03' in line:
            if 'X' in line:
                if 'Y' in line:
                    if len(a) == 2:
                        yup = line.find('E')
                        ...
                        if (yup >= 0):
                            ...
                            E_rem = line[yup:]
                            A_rem = line[:yup]
                        new_line = b.write(A_rem + ' A' + str(a[0] / sum(a)) + ' B' + str(a[1] / sum(a)) + ' C' + str(
                            a[2] / sum(a)) + ' ' + E_rem)
                    else:
    ...
    if add == 0:
        if line.startswith('G92'):
            if len(a) > 2:
                b.write('M165 A' + str(a[0] / sum(a)) + ' B' + str(a[1] / sum(a)) + ' C' + str(a[2] / sum(
                    a)) + '  ; a tot for each filament' + '\n' + 'G92 E0' + '\n' + 'G1 E30 F100     ; extrude 30mm' + '\n')
            else:
                b.write('M165 A' + str(a[0] / sum(a)) + ' B' + str(a[1] / sum(
                    a)) + '  ; a tot for each filament' + '\n' + 'G92 E0' + '\n' + 'G1 E30 F100     ; extrude 30mm' + '\n')
